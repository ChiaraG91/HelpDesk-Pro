    ...
    <div className="space-y-5">
      ...
      <div className="bg-[#111f30] border border-[#2d4060] rounded-xl overflow-hidden">
        {filtered.length === 0 ? (
          ...
          <>
            {/* Mobile: card list */}
            <div className="md:hidden divide-y divide-[#1a2e42]">
              {filtered.map((ticket) => (
                <div
                  key={ticket.id}
                  onClick={() => navigate(`/tickets/${ticket.id}`)}
                  className="px-4 py-4 hover:bg-[#1a2e42] cursor-pointer transition-colors"
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <p className="text-sm font-medium text-slate-200 leading-snug flex-1">{ticket.title}</p>
                    <StatusBadge status={ticket.status} />
                  </div>
                  <p className="text-xs text-slate-500 mb-3 line-clamp-2">{ticket.description.slice(0, 80)}...</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <PriorityBadge priority={ticket.priority} />
                      <span className="text-xs text-slate-500 capitalize">{ticket.category}</span>
                    </div>
                    <span className="text-xs text-slate-600">{formatDate(ticket.createdAt)}</span>
                  </div>
                </div>
              ))}
            </div>
